...
namespace Binance.Net.Clients.CoinFuturesApi
{
    ...
    public class BinanceClientCoinFuturesApi : RestApiClient, IBinanceClientCoinFuturesApi, IFuturesClient
    {
        ...
        internal BinanceClientCoinFuturesApi(ILogger logger, HttpClient? httpClient, BinanceRestOptions options)
            : base(logger, httpClient, ((BinanceEnvironment)options.Environment).CoinFuturesRestAddress!, options, options.CoinFuturesOptions)
        {
            Options = options;

            Account = new BinanceClientCoinFuturesApiAccount(this);
            ExchangeData = new BinanceClientCoinFuturesApiExchangeData(logger, this);
            Trading = new BinanceClientCoinFuturesApiTrading(logger, this);
        ...
        internal async Task<BinanceTradeRuleResult> CheckTradeRules(string symbol, decimal? quantity, decimal? quoteQuantity, decimal? price, decimal? stopPrice, FuturesOrderType type, CancellationToken ct)
        {
            ...
            if (Options.CoinFuturesOptions.TradeRulesBehaviour == TradeRulesBehaviour.None)
            ...
            if (symbolData.LotSizeFilter != null || symbolData.MarketLotSizeFilter != null && type == Enums.FuturesOrderType.Market)
            {
                ...
                if (minQty.HasValue && quantity.HasValue)
                {
                    ...
                    {
                        if (Options.CoinFuturesOptions.TradeRulesBehaviour == TradeRulesBehaviour.ThrowError)
                        {
            ...
            ArrayParametersSerialization? arraySerialization = null, int weight = 1, bool ignoreRateLimit = false) where T : class
        {
            ...
            if (!result && result.Error!.Code == -1021 && Options.CoinFuturesOptions.AutoTimestamp)
            {
        ...
        public override TimeSyncInfo? GetTimeSyncInfo()
            => new TimeSyncInfo(_logger, Options.CoinFuturesOptions.AutoTimestamp, Options.CoinFuturesOptions.TimestampRecalculationInterval, TimeSyncState);
